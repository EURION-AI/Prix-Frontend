...
const plans = [
  {
    ...
    features: ['5 automated PR fixes per month', '5 issue fixes per month', 'Public repositories', 'GitHub integration'],
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 xl:gap-8 max-w-6xl mx-auto">
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`relative flex flex-col p-6 lg:p-8 rounded-2xl border ${plan.popular ? 'border-primary/50 bg-primary/10 shadow-[0_0_40px_rgba(236,72,153,0.08)]' : 'border-white/[0.08] bg-white/[0.05]'} transition-all duration-300 hover:border-white/20 hover:-translate-y-1 will-change-transform`}
            >
              {plan.badge && (
